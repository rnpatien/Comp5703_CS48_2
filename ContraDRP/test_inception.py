...
parser.add_argument('--dataset', type=str, default='cifar10', help='RP data set')
...
for i in range(args.n_imgs):
    img=imread(str(data[i])).astype(np.float32).T 
    X.append(img)
X=np.array(X)
...
# run inference model
FIDscore,m2=fid_obj.FIDforImgs(X)
print("#  -- FID = %.5f" % FIDscore)
print("#  -- Av mean = %.5f " % (m2))
fwrite(OUT_FN, f'{FIDscore:.4f},{m2:.4f}')
